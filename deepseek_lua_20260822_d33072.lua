...
-- FIX: Proper tab creation with correct indexing
local function createTab(name, text)
    ...
    -- Store tab data
    local tabData = {button = tab, frame = scrollFrame, name = name}
    tabs[tab] = tabData
    table.insert(tabs, tabData) -- Also add to array for iteration
...
local function createToggleButton(text, yPos, defaultState, parent)
    ...
    -- Return frame, toggle button, and a function to get state
...
local function createKeybind(text, yPos, defaultKey, parent, callback)
    ...
    return frame, keyButton, function() return currentKey end
end
...
-- FIX: Proper category creation with getter/setter
...
local aimCat, setAimY = createCategory("AIMBOT", "🎯", mainTab)
local aimY = yPos
...
-- ESP Category
local espCat, setEspY = createCategory("ESP", "👁️", mainTab)
local espY = yPos
...
-- Movement Category
local moveCat, setMoveY = createCategory("MOVEMENT", "🏃", mainTab)
local moveY = yPos
...
-- Combat Category
local combatCat, setCombatY = createCategory("COMBAT", "⚔️", mainTab)
local combatY = yPos
...
-- Utility Category
local utilCat, setUtilY = createCategory("UTILITY", "🛠️", mainTab)
local utilY = yPos
...
local bladeCat, setBladeY = createCategory("BLADE BALL CHEATS", "⚔️", bladeTab)
local bladeY = yPos
...
local skinCat, setSkinY = createCategory("SKIN CHANGER", "🎨", skinTab)
local skinY = yPos
...
local potatoCat, setPotatoY = createCategory("POTATO GRAPHICS", "🥔", potatoTab)
local potatoY = yPos
